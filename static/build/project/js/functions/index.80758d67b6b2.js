...
function select(text,selectData,callback) {
  var data = [];
  ...
  return callback(text,data,selectData);
}

function selectCB(text,currentData,selectData) {
...
function checkKeywordExists(text) {
  ...
  $.ajax({
    ...
    data : {keyword : text},
    success : function(data)  {
       console.log('checkKeywordExists')
       console.log(data)
       if(data.exists && !data.active){

       }else {
         select(text,data,selectCB)
       }
    },
    error : function() {
